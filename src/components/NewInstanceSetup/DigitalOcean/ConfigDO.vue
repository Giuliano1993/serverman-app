...
const images = await DigitalOcean.getDistributions().then( res => res.map( d => ({label: d.name, value: d.id})));
...
const createServer = async () => {
  ...
  const res = await DigitalOcean.createDroplet(name.value, selectedSize.value, selectedImage.value);
  console.log(res);
